...
void DivideGraphHelper::DeriveNodeInputConnection(
    ...
    // 使用迭代器逐个插入元素到 std::set 中
    for (auto it = current_node->Inputs().begin(); it != current_node->Inputs().end(); ++it) {
             inputs.insert(*it);
    }
...
void DivideGraphHelper::DeriveNodeOutputConnection(
    ...
    for (auto it = current_node->Outputs().begin(); it != current_node->Outputs().end(); ++it) {
        outputs.insert(*it);
    }
...
std::pair<SimpleGraph, SimpleGraph> DivideGraphHelper::BuildSimpleGraph() {
    ...
    for (auto& info_kv : node_infos) {
        ...
        auto input_ids = node->Inputs() | ranges::views::transform([&](auto& s) { return node_infos[s].subgraph_num; })
                         | ranges::to_vector;
	std::set<std::int32_t> in_set;

        for (auto it = input_ids.begin(); it != input_ids.end(); ++it) {
            in_set.insert(*it);
        }	
