    ...
    m.def("search_policy",
          [](PyGraph& graph, std::vector<PyDevice> devices, const std::string& policy) -> pybind11::object {
              ...
              if (policy == "aware") {
                  ...
                  framework::CostGraph cost_graph = framework::ConvertGraphToCostGraph(*graph.GraphPtr());
                  framework::MergedCostGraph merged_cost_graph =framework::InitMergedCostGraph(cost_graph);
                  framework::NetworkxGenerator networkx_generator(merged_cost_graph);
                  networkx_generator.ConvertMergedCostGraph();
                  py::object networkx_graph = networkx_generator.GetNetworkxGraph();

                  framework::AwareInterface aware_interface(config_params, simulator_params, networkx_graph);
                  ...
                  return pybind11::cast(best_placement);
              }
              return pybind11::none();
          });
#ifdef VERSION_INFO
